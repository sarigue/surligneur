(function (global) {
    ...
    function bindEvents(el, scope) {
        el.addEventListener('keyup', scope.highlightHandler.bind(scope));
        el.addEventListener('touchend', scope.highlightHandler.bind(scope));
    }

    function unbindEvents(el, scope) {
        el.removeEventListener('keyup', scope.highlightHandler.bind(scope));
    ...
    /**
     * Returns 'black' or 'white' depending on which offers better contrast against the given CSS rgb() color.
    ...
    function contrastColor(cssColor) {
        ...
        if (!m) { return null; }
        var channels = [parseInt(m[1]), parseInt(m[2]), parseInt(m[3])].map(function (c) {
            c = c / 255;
    ...
    TextHighlighter.createWrapper = function (options) {
        ...
        var textColor = contrastColor(options.color);
        if (textColor !== null) {
